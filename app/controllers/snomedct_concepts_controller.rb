class SnomedctConceptsController < ApplicationController
	...
	def index
		...
		if params[:snomedct_id]
			@concepts = @concepts.includes(:descriptions).where('snomedct_id = ?', params[:snomedct_id])
		elsif params[:description]
			@concepts = @concepts.joins(:descriptions).where('term LIKE ?', "%#{params[:description]}%")
		end
